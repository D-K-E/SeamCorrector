...
class SceneCanvas(QtWidgets.QGraphicsScene):
    ...
    def __init__(self,
        ...
        self.activePointIndex = None
        self.oldPoint = None
        self.oldPointIndex = None
        if image:
            imw = image.width()
            imh = image.height()
    ...
    def insertNewPoint2ActivePolygon(self, selectedPoint: QtCore.QPoint,
                                     index):
        "Add given point to polygon"
        self.oldPointIndex = index
        self.oldPoint = self.activePolygon.at(index)
        self.activePoint = selectedPoint
        self.activePolygon.setPoint(selectedPoint, index)

    def makePolygonFromCoordinate(self, coordDict: dict):
        "Make a polygon from seam coordinates"
        seamarr = coordDict['seamCoordinates']
        points = []
        for i in range(seamarr.shape[0]):
            y, x = seamarr[i]
            point = QtCore.QPoint()
            point.setX(x)
            point.setY(y)
    ...
    def drawSceneData(self):
        ...
        #
        painter.end()
        for i, data in self.pointEditor.pointsData.items():
            if "seamCoordinates" in data:
                polygon = self.makePolygonFromCoordinate(data)
                polyf = QtGui.QPolygonF(polygon)
                item = QtWidgets.QGraphicsPolygonItem(polyf)
                self.addItem(item)
...
class AppWindowFinal(AppWindowInit):
    ...
    def __init__(self):
        ...
        # Main Window Events
        self.main_window.setWindowTitle("Seam Corrector using Coordinates")
    ...
    def addImage(self):
        ...
        if fdir:
            for impath in fdir[0]:
                self.addImage2Table(impath)

    def addCoordinates(self):
        "Add coordinate file to image on table"
        imageId = self.getImageIdFromTable()
        if imageId is None:
            return
        fdir = QtWidgets.QFileDialog.getOpenFileNames(
            self.centralwidget,
            "Select Coordinates", "", "Json (*.json")
        if fdir:
            for coordpath in fdir[0]:
                self.addCoordinateFile2Table(imageId, coordpath)

    def addCoordinateFile2Table(self, imageId,
                                coordpath: str):
        "Add coordinate file to image row"
        imPoint = self.imagePoint[imageId]
        imPoint['coordinates'] = {}
        imPoint['coordinates']['path'] = coordpath
        imPoint['coordinates']['name'] = os.path.basename(coordpath)
        item = QtWidgets.QTableWidgetItem(imPoint['coordinates']['name'])
        ...
        imPoint['coordinates']['index'] = itemindex
        self.imagePoint[imageId] = imPoint
